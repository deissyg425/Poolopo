...
# --- SETTINGS ---
START_DATE = datetime.date(2021, 1, 1) # STARTING FROM 2021
...
def run_hunt():
    ...
    # TRIPLE SOURCE CHECK (If one fails, it tries the next)
    sources = [
    ...
    if not domains:
        print(f"No data found for {target_date} anywhere. Skipping to next day.")
    else:
        processed = 0
        with open(MAGENTO_TECH_FILE, "a") as mt, open(MAGENTO_FASHION_FILE, "a") as mf, \
            ...
            for domain in domains:
                ...
                print(f"Checking: {clean}...", end=" ")
                res = check_site(clean)
                if res == "magento_tech":
                    print("!!! MAGENTO TECH !!!")
                    mt.write(f"{clean} (Added: {target_date})\n")
                elif res == "magento_fashion":
                    print("!!! MAGENTO FASHION !!!")
                    mf.write(f"{clean} (Added: {target_date})\n")
                elif res == "exclusive_tech":
                    print("!!! EXCLUSIVE TECH !!!")
                    ef.write(f"{clean} (Added: {target_date})\n")
                else: print("No.")
                
                hf.write(clean + "\n")
                processed += 1

    # Save progress ALWAYS to keep the loop moving forever
